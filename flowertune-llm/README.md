...
The base model is selectable per run. The models prepared for the multi-node
FSDP/LoRA path are [Qwen3-14B](https://huggingface.co/Qwen/Qwen3-14B),
[Qwen3-8B](https://huggingface.co/Qwen/Qwen3-8B), and
[OpenLLaMA 3Bv2](https://huggingface.co/openlm-research/open_llama_3b_v2).
Override `model.name` directly from the command line:

```bash
# Qwen3-14B (the current default)
flwr run . --run-config "model.name='Qwen/Qwen3-14B'" --stream

# Qwen3-8B
flwr run . --run-config "model.name='Qwen/Qwen3-8B'" --stream

# OpenLLaMA 3Bv2
flwr run . --run-config "model.name='openlm-research/open_llama_3b_v2'" --stream
```

Both model caches must be present on each client cluster before offline
training starts. You can still override other configuration parameters:
